...
pub fn generate_json_from_input(input: &str) -> Result<String> {
    let input_type = crate::input::detect_input_type(input)?;

    let ym2151_json = match input_type {
        InputType::MmlString(mml) | InputType::MmlFile(mml) => convert_mml_to_json(&mml)?,
        InputType::MidFile(smf_data) => convert_smf_to_json(&smf_data)?,
...
fn convert_mml_to_json(mml: &str) -> Result<String> {
    ...
    convert_smf_to_json(&smf_data)
}

/// Converts SMF data to YM2151 JSON
fn convert_smf_to_json(smf_data: &[u8]) -> Result<String> {
